...
int SPEED_A = 200; //velocidades
int SPEED_B = 200;

int Arma = 0; // pin 0 va conectado al motor del arma
...
void motors_setup()
{
  ...
  // inicializamos el motor del arma apagado
  pinMode(Arma, OUTPUT);
...
// Motor Arma
// PrenderArma activa el motor
void PrenderArma(void) {
  digitalWrite(Arma, HIGH);
}
// ApagarArma apaga el motor
void ApagarArma(void) {
  digitalWrite(Arma, LOW);
}
