...
void handleButtonRequest(void) {
  // AdltAtrs indica si se quiere retroceder o avanzar y puede tomar los valores 1, -1 y 0
  int AdltAtrs = server.arg("AdltAtrs").toInt();
  // DerIzq indica si se quiere girar a la izquierda o derecha y puede tomar los valores 1, -1 y 0
  int  DerIzq = server.arg("DerIzq").toInt();
  // arma indica si se quiere activar o desactivar el arma y puede tomar los valores 0 y 1
  int arma = server.arg("arma").toInt();

  

  if (arma == 0) {
    Serial.println("arma prendida");
    PrenderArma();
  }
  if (arma == 1) {
    Serial.println("arma apagada");
    ApagarArma();
  }


  // Set the motor speeds based on the calculated values
  // Update this part based on your motor control logic
  // Example: analogWrite(leftMotorPin, leftSpeed);
